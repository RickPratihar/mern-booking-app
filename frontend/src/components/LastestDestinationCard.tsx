...
const LatestDestinationCard = ({ hotel }: Props) => {
  return (
    <Link
      to={`/detail/${hotel._id}`}
      className="group relative cursor-pointer overflow-hidden rounded-2xl bg-white border border-slate-100 shadow-[0_4px_25px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_35px_rgba(0,0,0,0.12)] hover:-translate-y-1.5 transition-all duration-300 flex flex-col h-full"
    >
      ...
      <div className="p-5 flex flex-col flex-1 gap-2 bg-white rounded-b-2xl">
        ...
        {/* Name */}
        <h3 className="text-lg font-bold text-slate-800 tracking-tight group-hover:text-blue-600 transition-colors duration-200 line-clamp-1 mt-0.5">
          {hotel.name}
        </h3>
